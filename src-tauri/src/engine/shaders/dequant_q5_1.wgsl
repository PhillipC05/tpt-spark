...
//
// Q5_1 layout (per 32-element block, 24 bytes):
//   bytes  0-1 : d (f16 scale)
//   bytes  2-3 : m (f16 min)
//   bytes  4-7 : qh[4] — 32 high bits (bit i of qh[i/8])
//   bytes  8-23: qs[16] — 32 × 4-bit low nibbles (split layout as Q4_0)
//
// 5-bit value: q5 = lo4 | (high_bit << 4)  → range 0..31
// Dequant: x[i] = d * q5 + m              (all positive, no centering)
//
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Q5_1 block = 24 bytes, 32 elements.
    let block_idx = elem_idx / 32u;
    let lane      = elem_idx % 32u;
    let bb        = block_idx * 24u;

    let d = decode_f16(read_byte(bb),      read_byte(bb + 1u));
    let m = decode_f16(read_byte(bb + 2u), read_byte(bb + 3u));

    // High bit: qh at bytes 4-7.
    let qh_byte  = bb + 4u + lane / 8u;
    let high_bit = (read_byte(qh_byte) >> (lane % 8u)) & 1u;

    // Low 4 bits: bytes 8-23, split layout.
    let qs_byte = bb + 8u + (lane % 16u);
    let raw     = read_byte(qs_byte);
    let lo4     = select(raw & 0x0Fu, (raw >> 4u) & 0x0Fu, lane >= 16u);

    let q5 = lo4 | (high_bit << 4u);   // 0..31
    out_f32[elem_idx] = d * f32(q5) + m;
}
